...
public class VScriptEditor extends JDialog implements ActionListener
{
	Base base;
	JToolBar editorTb;
	...
	String currentFile;
	VScriptEditor thisEditor;
	String arguement="";
	
	JTabbedPane scriptEditorTp;
	JPanel variablePanel;
	VScriptEditor(Base b)
	{
		
		super(b, "V-Script Editor",false);
		base=b;
		...
		if(ae.getSource()==runBt)
		{
			...
			for(int i=0;i<st.length();i++)
			{
				...
				if(method.length()>0 && argsReadable==false)
				{
					...
					if(method.equals("start timeline"))
					{
						vToolBox.setTimeLine();
					}
					
					if(method.equals("setTurningRadius"))
					{
						try
						{
							float args=Float.parseFloat(arguement);
							base.turningRadius=(int)(args*base.scale);
							System.out.println("scaled turning radius is"+base.turningRadius);
							//System.exit(0);							
						}
						catch(Exception e){}
					}
					...
					if(method.equals("setDaShape"))
					{
						try
						{
							int num =Integer.parseInt(arguement);
							System.out.println(num);
							//System.exit(0);
						 	if(num==1)
						 	{
						 		base.dAShapeCb.setSelectedIndex(0);
						 	}
						 
						 	if(num==2)
						 	{
						 		base.dAShapeCb.setSelectedIndex(1);
						 	}
						}
						catch(Exception e){}
					}
					
					if(method.equals("aads_deployment"))
					{
						try
						{
							   // String args[]=arguement.split(",");
							   // float rs=Float.parseFloat(arguement);
							    //System.out.println("Error range ="+rs);
							    
							    vToolBox.aadsDeployment();
					...
					if(method.equals("setMaxDeploymentError"))
					{
						try
						{
							float mde=Float.parseFloat(arguement);
							base.maxDeploymentError=mde*base.scale;
							
							System.out.println("MDE:"+ base.maxDeploymentError);
						//System.exit(0);							
						}catch(Exception e){}
					}
					...
					if(method.equals("open_deployment_monitor_dialog"))
					{
					...
					if(method.equals("open_aquatic_deployment_monitor_dialog"))
					{
						AquaticDeploymentControlDialog aDCD=new AquaticDeploymentControlDialog(base);
						aDCD.setBounds(800,200,600,500);
...
/*					if(method.equals("update_bs_cluster_list"))
					...
					if(method.equals("setDeploymentLoc"))
					{
						try
						{
							//System.exit(0);
							...
							base.candidateLoc=new Location(xx, yy, base);
						}catch(Exception e){e.printStackTrace(); }
					}
	...
	new Thread(r).start();
		...
		if(ae.getSource()==saveAsBt)
		{
			...
			//saveScript("test1",st);
			SaveAsDialog sd=new SaveAsDialog(base,this);
	...
	void saveScript(String fileName,String scriptSt)
	{
		   ...
		   try{
		      ...
		      OutputStream os = new FileOutputStream("script/"+fileName);
	...
	void getScript(String fileName, JTextPane scriptPa)
	{
		try
		{
	      InputStream is = new FileInputStream("script/"+fileName);
